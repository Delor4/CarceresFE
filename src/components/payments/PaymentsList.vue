      ...
      <b-card class="payments_list">
        ...
        <b-list-group-item v-for="model in models" v-bind:key="model.id">
          ...
          >
            ...
            Miejsce: {{ subscriptions[model.id].place_id }} Samochód: {{ subscriptions[model.id].car_id }}
            Start: {{ subscriptions[model.id].start }} End: {{ subscriptions[model.id].end }}
...
export default {
  ...
  methods: {
    ...
    async loadModels() {
      this.subscriptions = await this.api.getAll(this.api.getSubscriptions);
